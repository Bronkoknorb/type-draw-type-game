...
class GameRoundsGeneratorTests {
  ...
  private void checkAllPlayersPlayEveryStory(int[][] rounds) {
    Set<Integer> allStoryNumbers = IntStream.range(0, rounds.length)
      .boxed()
    ...
    for (int p = 0; p < rounds.length; p++) {
      ...
      Assertions.assertEquals(
        allStoryNumbers,
        storiesOfPlayer,
        "Player needs to play all stories"
      );
    }
  }
  ...
  private void assertAllRoundsContainEveryStory(int[][] rounds) {
    Set<Integer> allStoryNumbers = IntStream.range(0, rounds.length)
      ...
      .collect(Collectors.toSet());
    for (int[] round : rounds) {
      Set<Integer> storiesInRound = Arrays.stream(round)
        .boxed()
        .collect(Collectors.toSet());
      Assertions.assertEquals(
        allStoryNumbers,
        storiesInRound,
        "Round needs to contain all stories"
      );
    }
  }
  ...
  private void assertRoundsAndAssignmentLengths(
    int numberOfPlayers,
    int[][] rounds
  ) {
  ...
  private void checkThatGameIsPerfect(
    int numberOfPlayers,
    int[][] transitions
  ) {
    Set<Integer> allPlayerNumbers = IntStream.range(0, numberOfPlayers)
      .boxed()
      .collect(Collectors.toSet());
    for (int p = 0; p < numberOfPlayers; p++) {
      ...
      Set<Integer> allPlayerNumbersWithoutThemselves = new HashSet<>(
        allPlayerNumbers
      );
      allPlayerNumbersWithoutThemselves.remove(p);
      Assertions.assertEquals(
        allPlayerNumbersWithoutThemselves,
        transitionsOfPlayer,
        "Player needs to transition (hand a story over) to every other player"
      );
    }
  }
  ...
  private int[][] calculateTransitions(int[][] rounds) {
    ...
    for (int r = 0; r < rounds.length - 1; r++) {
      ...
      List<Integer> nextRoundAsList = Arrays.stream(nextRound)
        .boxed()
  ...
  void generateThrowsOnIllegalArgument() {
    Assertions.assertThrows(IllegalArgumentException.class, () ->
      GameRoundsGenerator.generate(-1)
    );
    Assertions.assertThrows(IllegalArgumentException.class, () ->
      GameRoundsGenerator.generate(0)
    );
    Assertions.assertThrows(IllegalArgumentException.class, () ->
      GameRoundsGenerator.generate(1)
    );
